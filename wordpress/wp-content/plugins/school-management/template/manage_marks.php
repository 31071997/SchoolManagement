...

{
...

	{
		...
		<div id="message" class="alert_msg alert alert-success alert-dismissible " role="alert">
			...
			<p><?php esc_attr_e('Marks Added successfully.','school-mgt');?>
...

	{
...

		{
			...
			<div id="message" class="alert_msg alert alert-success alert-dismissible " role="alert">
				...
				<p><?php esc_attr_e('Marks Updated successfully.','school-mgt');?>
...
<script type="text/javascript">
...

	{
		...
		$('#Add_marks_form').validationEngine({promptPosition : "bottomLeft",maxErrorsPerField: 1});
...
<div class="panel-body panel-white frontend_list_margin_30px_res">  
	...
	<ul class="nav nav-tabs panel_tabs flex-nowrap margin_left_1per" role="tablist">
		...
		<li class="<?php if($active_tab=='result'){?>active<?php }?>">			
			...
			<a href="?dashboard=user&page=manage_marks&tab=result" class="padding_left_0 tab <?php echo $active_tab == 'result' ? 'active' : ''; ?>">
		...
		<li class="<?php if($active_tab=='multiple_subject_marks'){?>active<?php }?>">			
			...
			<a href="?dashboard=user&page=manage_marks&tab=multiple_subject_marks" class="padding_left_0 tab <?php echo $active_tab == 'multiple_subject_marks' ? 'active' : ''; ?>">
		...
		<li class="<?php if($active_tab=='export_marks'){?>active<?php }?>">
			...
			<a href="?dashboard=user&page=manage_marks&tab=export_marks" class="padding_left_0 tab <?php echo $active_tab == 'export_marks' ? 'active' : ''; ?>">
					...
					<div class="form-body user_form">
						...
						<div class="row">
							...
							<div class="col-md-6 input error_msg_left_margin">
								...
								<select name="subject_id" id="subject_list" class="line_height_30px form-control validate[required] text-input">
				...
				<div class="clearfix panel-body">
					...
					<form method="post" class="mt-3 form-inline" id="marks_form" enctype="multipart/form-data">  
						...
						{ 
										...
										<div class="form-group input">
											...
											<div class="col-md-12 form-control res_rtl_height_50px">	
												...
												<div class="col-sm-12 csv_file_check">
													...
													<input type="file" name="csv_file" id="csv_file" class="d-inline file_validation" />
							...
							<input type="submit" name="upload_csv_file" value="<?php esc_attr_e('Fill data from CSV File','school-mgt');?>" class="fill_data btn save_btn_1" /> 

							<label for="" class="margin_top_20px padding_top_25px_res whitespace_initial">
						...
						<div class="table-responsive">
							...
							<table class="table col-md-12">
...

								{
									...
									if(isset($csv)){
										?>
										<input type="text" name="marks_<?php echo $user->ID;?>" value="<?php echo $csv[$key]['marks'];?>" class="form-control validate[required] text-input">
										<?php
									}
									else{
										?>
										<input type="text" name="marks_<?php echo $user->ID;?>" value="<?php echo $marks;?>" class="form-control validate[required] text-input">
										<?php
									}
									...
									echo '<button type="submit" name="add_mark" value="'.$user->ID.'" class="save_btn btn btn-success">'.$button_text.'</button>';	
...

					{
						...
						<div class="col-sm-6 margin_top_15px">
							...
							<input type="submit" class="save_btn btn btn-success table_bottom_margin_rtl" name="save_all_marks" value="<?php esc_attr_e('Update All Marks','school-mgt')?>">
