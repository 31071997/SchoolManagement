...
<div class="mailbox-content">
	...
	if(!empty($message))
	{
		...
		<div class="table-responsive"  id="sentbox_table">
			...
			<table class="table" id="sent_list">
					...
					{ 	
						...
						<tr>
							...
							<td><a href="?dashboard=user&page=message&tab=view_message&from=sendbox&id=<?php echo $msg_post->ID;?>" class="text_decoration_none">
								...
								?>
								</span></a> <i class="fa fa-info-circle fa_information_bg" data-toggle="tooltip" data-placement="top" title="<?php _e('Message For','school-mgt');?>" ></i>		
							</td>
							<td>
							...
							<td>
								<a href="?dashboard=user&page=message&tab=view_message&from=sendbox&id=<?php echo $msg_post->ID;?>" class="text_decoration_none">
									...
									?>
									<?php if(mj_smgt_count_reply_item($msg_post->ID)>=1){?><span class="badge badge-success pull-right"><?php echo mj_smgt_count_reply_item($msg_post->ID);?></span><?php } ?>
								</a> <i class="fa fa-info-circle fa_information_bg" data-toggle="tooltip" data-placement="top" title="<?php if(!empty($msg_post->post_title)){ echo $msg_post->post_title;}else{_e('Subject','school-mgt'); }?>" ></i>
							</td>
							<td>
								<a href="?dashboard=user&page=message&tab=view_message&from=sendbox&id=<?php echo $msg_post->ID;?>" class="text_decoration_none">
								<?php
								...
								?>
								</a> <i class="fa fa-info-circle fa_information_bg" data-toggle="tooltip" data-placement="top" title="<?php if(!empty($msg_post->post_content)){echo $msg_post->post_content;}else{_e('Description','school-mgt');}?>" ></i>
							</td>
							<td>	
								<?php
								...
								if(!empty($attchment))
								{
									...
									{	
										?>
										<a target="blank" href="<?php echo content_url().'/uploads/school_assets/'.$attchment_data; ?>" class="btn btn-default"><i class="fa fa-eye"></i><?php esc_attr_e('View Attachment','school-mgt');?></a></br>
										<?php
									}
								}
								else
