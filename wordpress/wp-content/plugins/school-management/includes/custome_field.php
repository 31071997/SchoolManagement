...
{  /*<---START Lmgt_custome_field  CLASS--->*/
	...
	function mj_smgt_add_custom_field($custome_data) 
	{
		...
		$custom_field_data['field_label']=stripslashes($custome_data['field_label']);	
		...
		{ 	
			...
			school_append_audit_log(''.esc_html__('Custom Field Updated','hospital_mgt').'',get_current_user_id(),get_current_user_id(),'edit',$_REQUEST['page']);
		...
		{ 
			...
			school_append_audit_log(''.esc_html__('Custom Field Added','hospital_mgt').'',get_current_user_id(),get_current_user_id(),'insert',$_REQUEST['page']);			
	...
	function mj_smgt_get_all_custom_field_data_own()
	{
		...
		global $wpdb;
		$wpnc_custom_fields = $wpdb->prefix . 'custom_field';
		$get_custom_field_data = $wpdb->get_results("SELECT * FROM $wpnc_custom_fields where created_by=$created_by AND field_visibility!=2 ORDER BY created_at DESC");
		return $get_custom_field_data;
	}
	...
	function mj_smgt_get_single_custom_fields_id_meta_value($module_record_id) 
	{	
		global $wpdb;
		$wpnc_custom_field_metas = $wpdb->prefix . 'custom_field_metas';
		$get_data = $wpdb->get_results("SELECT * FROM $wpnc_custom_field_metas where module_record_id='$module_record_id'");
		return $get_data;
	}
	
	//Get Single Custom field_value value BY custom_fields_id Function//
	function mj_smgt_get_single_field_value_meta_value($custom_field_id) 
	{	
		global $wpdb;
		$wpnc_custom_field_metas = $wpdb->prefix . 'custom_field_metas';
		$get_data = $wpdb->get_results("SELECT * FROM $wpnc_custom_field_metas where custom_fields_id= '$custom_field_id'");
		return $get_data;
	}

	function mj_smgt_get_single_field_value_meta_value_by_filed_and_student_id($student_id,$custom_field_id) 
	{	
		
		global $wpdb;
		$wpnc_custom_field_metas = $wpdb->prefix . 'custom_field_metas';
		$get_data = $wpdb->get_row("SELECT field_value FROM $wpnc_custom_field_metas where module_record_id ='$student_id' AND custom_fields_id= '$custom_field_id'");
