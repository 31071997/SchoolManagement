	...
	public static function student_dashboard( $request ) {
		...
		try {
			...
			foreach ($notices as $notice) {
				...
				// Check if notice_data does not exist or is empty or has empty arrays for all keys
				$is_empty_notice_data = !isset($notice_data) || empty($notice_data) ||
					(empty($notice_data['classes']) &&
					 empty($notice_data['sections']) &&
					 empty($notice_data['students']));

				if ($is_empty_notice_data) {
					// If notice_data does not exist or is empty, add the notice without checking conditions
