			...
			if ( $current_school = $user_info['current_school'] ) {
				if ( $current_school['is_active'] === '1' ) {
					...
					if ( in_array( $role, array_keys( WLSM_M_Role::get_roles() ) ) ) {
						...
						// Academic - Group.
						if ( WLSM_M_Role::check_permission( array( 'manage_classes', 'view_subjects', 'view_timetable', 'manage_attendance', 'manage_study_materials', 'manage_homework', 'manage_notices', 'manage_student_leaves', 'manage_events', 'manage_live_classes' ), $permissions ) ) {
						...
						// Student - Group.
						if ( WLSM_M_Role::check_permission( array( 'manage_admissions', 'manage_admissions', 'manage_promote', 'manage_transfer_student', 'view_certificates', 'send_notifications' ), $permissions ) ) {
