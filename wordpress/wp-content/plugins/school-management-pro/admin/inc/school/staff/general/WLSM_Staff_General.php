...
class WLSM_Staff_General
{
	public static function get_class_sections() {
		$current_user = WLSM_M_Role::can(array('manage_admissions', 'view_students', 'manage_invoices', 'manage_transfer_student', 'view_certificates', 'manage_library', 'manage_transport', 'manage_homework', 'manage_exams', 'manage_admins', 'manage_employees', 'manage_student_leaves', 'manage_attendance'));
		...
		try {
			...
			if (isset($assigned_subjects) && isset($assigned_subjects['school']) && $assigned_subjects['school']['role'] === 'admin') {
				$subject = WLSM_M_Staff_Class::fetch_subject_query_by_class_id($school_id, $class_id);
			} elseif (isset($assigned_subjects) && !current_user_can('administrator')) {
				$admin_id = $assigned_subjects['school']['admin_id'];
				$subject = WLSM_Helper::getAssignedSubjects($admin_id);
			} elseif (empty($assigned_subjects)) {
				$subject = WLSM_M_Staff_Class::fetch_subject_query_by_class_id($school_id, $class_id);
			}
	...
	{
		$current_user = WLSM_M_Role::can(array( 'view_timetable', 'manage_live_classes'));
		...
		try {
			...
			if (isset($current_user) && !current_user_can('administrator')) {
	...
	{
		$current_user = WLSM_M_Role::can(array('view_students'));
...
?>
	...
	public static function print_id_card() {
		$current_user = WLSM_M_Role::can('view_students', 'manage_transport');
	...
	{
		$current_user = WLSM_M_Role::can('edit_certificates');
